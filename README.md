...
反代启动后，双击打开 `task-panel.html`（会用浏览器打开）：

1. 点「检查连接」，显示绿色"已连接 · 池内 key 总数 N"即正常
2. 输入一句话任务（例：`打开 example.com，告诉我页面标题`），选模型，点「提交任务」
3. 云端浏览器开始干活，面板每 5 秒刷新状态，几分钟后出结果

面板走本反代的 `/api/v4/runs`（v4 一次性任务），key 池自动轮换，结果带 token 用量和花费。
